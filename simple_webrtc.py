...
def preprocess_frame_gpu(image_bgr):
    ...
    img_tensor = torch.from_numpy(img_rgb).to(DEVICE, non_blocking=True).permute(2, 0, 1).float()
        ...
        with self.lock:
            ...
            if self.click_queue:
                while self.click_queue:
                    ...
                    target_obj_id = determine_target_obj_id(cx, cy, self.last_masks_cache)
                    if target_obj_id is None:
                        if clabel == 1: # Only support include for now as requested
                            if self.active_objects:
                                target_obj_id = max(self.active_objects.keys()) + 1
                            else:
                                target_obj_id = 0
                            self.active_objects[target_obj_id] = {'points': [], 'labels': []}
                        else: continue
            ...
            if new_clicks_triggered_reinit or (not self.is_tracking and self.active_objects):
                if not self.active_objects:
                    ...
                    for obj_id in sorted(self.active_objects.keys()):
                        ...
                        point_coords = np.array(data['points'], dtype=np.float32)
                    ...
                    if temp_masks_list:
                        ...
                        for i, oid in enumerate(current_display_obj_ids):
                            if i < len(masks_np): self.last_masks_cache[oid] = masks_np[i].squeeze()

            elif self.is_tracking and self.active_objects:
                predictor.append_frame(self.inference_state, frame_tensor)
                self.frame_idx += 1
                out_masks = predictor.track_new_frame(self.inference_state, self.frame_idx)
