"""
EdgeTAM Backend WebRTC Server
基于 simple_webrtc.py 的界面，使用简化的单对象追踪逻辑
"""
...
def get_mask_overlay(image, mask_tensor, alpha=0.5):
    ...
    # mask_tensor shape: (1, 1, H, W), dtype: float32
    mask_np = mask_tensor.cpu().numpy()
    mask = mask_np[0, 0]  # 提取 (H, W)
...
class EdgeTAMTrack(MediaStreamTrack):
    ...
    def __init__(self):
        super().__init__()
        logger.info("Initializing EdgeTAMTrack (single object tracking)...")
...
async def index():
    """提供 HTML 界面（完全参考 simple_webrtc.py）"""
    return """
    <html>
    <head>
...
if __name__ == "__main__":
    logger.info("Starting EdgeTAM WebRTC Backend Server on http://0.0.0.0:7860")
    logger.info("Features:")
    logger.info("  - Single object tracking only")
    logger.info("  - WebRTC video streaming at 640x480 @ 30 FPS")
    logger.info("  - Access from browser: http://<IP>:7860")
    logger.info("  - Click video to start tracking, Reset button to track new object")
